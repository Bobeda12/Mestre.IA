...
const getLocalImage = (type: 'classes' | 'races', name: string) => {
    const filename = name.toLowerCase()
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        .replace(/\s+/g, '-'); 
    return `/assets/${type}/${filename}.jpg`;
};

const getPointCost = (score: number) => {
    if (score <= 8) return 0;
    const costs: Record<number, number> = {9:1, 10:2, 11:3, 12:4, 13:5, 14:7, 15:9};
...
export default function CharacterCreation({ onCharacterCreated }: CharacterCreationProps) {
  ...
  useEffect(() => {
    if (selectedRace) {
        axios.get(`http://127.0.0.1:8000/options/races/${selectedRace}`).then(res => {
            setRaceData(res.data);
            setFreePointsAllocation({}); // Reseta alocação se trocar raça
        }).catch(() => {});
    }
  }, [selectedRace]);
  ...
  useEffect(() => {
    if (step === 4 && name && gender && selectedRace && selectedClass) {
        const prompt = `fantasy rpg character portrait of a ${gender} ${selectedRace} ${selectedClass}, highly detailed face, looking at camera, dnd art style, masterpiece, sharp focus, dark fantasy background`;
  ...
  const getFinalAttribute = (attr: string) => {
      ...
      const freeBonus = freePointsAllocation[attr] || 0;
      return base + fixedBonus + freeBonus;
  };
  ...
  // Helpers de UI
  const activeImage = step === 5 && finalImageUrl ? finalImageUrl :
                      step === 2 && selectedClass ? getLocalImage('classes', selectedClass) :
                      step === 1 && selectedRace ? getLocalImage('races', selectedRace) :
                      "/assets/background-default.jpg";

  const activeTitle = step === 5 ? name :
                      step === 1 ? (selectedRace || "Linhagem") :
                      step === 2 ? (selectedClass || "Vocação") :
                      step === 3 ? "Identidade" : "Atributos";
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-black font-sans text-gray-100">
      ...
      {finalImageUrl && <img src={finalImageUrl} className="hidden" alt="preload" />}

      <button onClick={() => navigate('/')} className="absolute top-4 left-4 z-50 text-gray-500 hover:text-white flex items-center gap-2">
      ...
      <div className="w-1/3 h-full flex flex-col bg-gray-900 border-r border-gray-800 z-20 shadow-2xl relative">
        <div className="p-6 border-b border-gray-800 bg-black/40 mt-10">
           <h1 className="text-3xl font-rpg text-rpg-gold flex items-center gap-2">
             <Crown className="text-red-600"/> CRIAÇÃO
           </h1>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar pb-24">
            ...
            {step === 3 && (
                <div className="p-4 space-y-6 animate-fade-in">
                    <div><label className="text-rpg-gold font-rpg block mb-2">Nome</label><input type="text" className="w-full bg-black/50 border border-gray-600 p-3 rounded text-white text-lg outline-none" placeholder="Ex: Vorag" value={name} onChange={e => setName(e.target.value)} /></div>
                    <div>
                        <label className="text-rpg-gold font-rpg block mb-2">Gênero</label>
                        <div className="grid grid-cols-3 gap-2">{["Masculino", "Feminino", "Outro"].map(g => (<button key={g} onClick={() => setGender(g)} className={`p-2 rounded border text-sm transition-all ${gender === g ? 'bg-rpg-gold text-black border-rpg-gold' : 'bg-black border-gray-700 hover:border-gray-500'}`}>{g}</button>))}</div>
                    </div>
                    <div><label className="text-rpg-gold font-rpg block mb-2">História</label><textarea className="w-full bg-black/50 border border-gray-600 p-3 rounded text-white h-24 outline-none resize-none" placeholder="Sua origem..." value={history} onChange={e => setHistory(e.target.value)} /></div>
                </div>
            )}
                    ...
                    {maxFreePoints > 0 && (
                        <div className={`p-3 rounded border text-center ${usedFreePoints === maxFreePoints ? 'bg-green-900/20 border-green-700' : 'bg-rpg-gold/10 border-rpg-gold'}`}>
                            <span className="text-sm font-bold text-gray-200 block mb-1">
                                <Sparkles size={12} className="inline mr-1"/> Bônus Racial Extra
                            </span>
                            <span className={`text-xl font-rpg ${usedFreePoints === maxFreePoints ? 'text-green-400' : 'text-rpg-gold'}`}>
                                {usedFreePoints}/{maxFreePoints}
                            </span>
                            <p className="text-[10px] text-gray-400 mt-1">Selecione {maxFreePoints} atributos abaixo para ganhar +1</p>
                        </div>
                    )}
                            ...
                            return (
                                <div key={attr} className="flex items-center justify-between bg-gray-900/50 p-2 rounded border border-gray-800 hover:border-gray-600">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button onClick={() => handleAttributeChange(attr, 1)} className="w-8 h-8 rounded bg-gray-800 hover:bg-green-900 flex items-center justify-center text-white disabled:opacity-30" disabled={attributes[attr] >= 15 || pointsRemaining === 0}><Plus size={14}/></button>
                                    </div>
                                    <div className="text-[10px] text-gray-500 w-12 text-right">-{getPointCost(attributes[attr] + 1) - getPointCost(attributes[attr])}</div>
                                </div>
                            );
                        })}
                    </div>
             ...
             <div className="w-[55%] p-8 flex flex-col relative overflow-y-auto custom-scrollbar">
                 ...
                 {step === 5 ? (
                     <>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                             {Object.keys(attributes).map(attr => (
                                 <div key={attr} className="bg-gray-800/40 border border-gray-700 rounded p-2 text-center relative">
                                     <span className="block text-[10px] text-gray-500 uppercase font-bold mb-1">{formatAttribute(attr)}</span>
                                     <span className="block text-xl text-white font-rpg">{getFinalAttribute(attr)}</span>
                                     <div className="absolute -top-2 -right-2 w-6 h-6 bg-gray-700 rounded-full flex items-center justify-center text-[10px] text-blue-300 font-bold border border-gray-600 shadow-sm">{formatModifier(getModifierValue(getFinalAttribute(attr)))}</div>
                                 </div>
                             ))}
                        </div>
                     </>
                     ...
                     <div className="space-y-6 animate-fade-in">
                        {currentDetails.quote && <p className="text-xl text-rpg-gold italic font-serif text-center px-4">"{currentDetails.quote}"</p>}
                        <div className="text-gray-300 leading-relaxed text-justify bg-black/20 p-4 rounded border border-gray-800">{currentDetails.descricao}</div>
                        {step === 1 && currentDetails.bonus_atributos && (
                            <div className="mt-4">
                                <span className="text-xs text-gray-500 uppercase tracking-widest block mb-2">Bônus de Atributo</span>
                                <div className="flex gap-2 flex-wrap">
                                    {Object.entries(currentDetails.bonus_atributos)
                                        .filter(([k]) => k !== 'livre_escolha') // Não mostra "LIVRE" aqui, pois é dinâmico agora
                                        .map(([k,v]) => (
                                        <span key={k} className="text-xs bg-blue-900/40 px-3 py-1 rounded text-blue-200 border border-blue-800 uppercase flex items-center gap-1"><Zap size={10} /> +{v as number} {formatAttribute(k)}</span>
                                    ))}
                                    {/* Mostra tag especial se tiver livre escolha */}
                                    {currentDetails.bonus_atributos.livre_escolha > 0 && (
                                         <span className="text-xs bg-purple-900/40 px-3 py-1 rounded text-purple-200 border border-purple-800 uppercase flex items-center gap-1"><Sparkles size={10} /> +{currentDetails.bonus_atributos.livre_escolha} Escolha</span>
                                    )}
                                </div>
                            </div>
                        )}
                        {step === 2 && currentDetails.dado_vida && <div className="mt-4 flex gap-4"><span className="flex items-center gap-2 text-gray-300 bg-gray-800 px-3 py-1 rounded border border-gray-700"><Heart size={14} className="text-red-500"/> Vida Base: d{currentDetails.dado_vida}</span></div>}
                     </div>
                 ) : (
                 ...
                 {canProceed && step < 5 && (
                    <div className="mt-auto pt-6 flex justify-end">
                        <button onClick={() => setStep(s => s + 1)} className="bg-rpg-gold hover:bg-white text-black p-3 rounded-full shadow-lg transition-all hover:scale-110 flex items-center gap-2 font-bold px-6">PRÓXIMO <ChevronRight size={20} /></button>
                    </div>
                 )}
             </div>
         </div>
