  ...
  return (
    <section className="adventure-stage" aria-label="Cena interativa da aventura" aria-busy={p.ocupado}>
      <header className="adventure-stage__header">
        ...
        <span className="adventure-stage__level"><PixelIcon name="estrela" size={13} /> NV. {p.nivel}</span>
      </header>
      ...
      <div className={`adventure-world adventure-world--${tema} ${noite ? 'adventure-world--noite' : ''}`}>
        ...
        <div className="adventure-world__party">
          <button type="button" className={`adventure-actor adventure-actor--hero ${caido ? 'adventure-actor--dead' : ''}`}
            ...
            <span className="adventure-actor__tag">{p.escondido ? 'OCULTO' : p.bonusDefesa > 0 ? `DEFESA +${p.bonusDefesa}` : p.classe}</span>
            ...
            <span className="adventure-actor__name">{p.nome || 'Herói'}</span>
            <span className="adventure-actor__life">{p.hp}/{p.hpMax} PV</span>
          ...
          <div className="adventure-world__opponents">
            {p.inimigos.map((inimigo, indice) => (
              <button type="button" key={inimigo.nome}
                ...
                <span className="adventure-actor__tag">{inimigo.hp <= 0 ? 'DERROTADO' : inimigo.intencao || 'OBSERVANDO'}</span>
                ...
                <span className="adventure-actor__shadow" aria-hidden="true" />
                <span className="adventure-actor__name">{inimigo.nome}</span>
                <span className="adventure-actor__life">{inimigo.hp}/{inimigo.max_hp} PV · DEF {inimigo.ca}</span>
                <PixelBar value={inimigo.hp} max={inimigo.max_hp} segments={8} colorClass="bg-red-500" />
                {Object.entries(inimigo.efeitos ?? {}).filter(([, duracao]) => duracao > 0).map(([efeito, duracao]) => <span className="adventure-actor__effect" key={efeito}>{efeito.replaceAll('_', ' ')} · {duracao}</span>)}
              </button>
            ))}
            {!p.combate && !p.inimigos.length && <div className="adventure-world__landmark" aria-hidden="true"><PixelIcon name={tema === 'ruinas' ? 'pergaminho' : 'bau'} size={44} /><span>O mundo espera sua decisão</span></div>}
          </div>
        </div>
        {(p.cena?.interacoes.length ?? 0) > 0 && <div className="adventure-world__objects" aria-label="Objetos e caminhos da cena">
        ...
        {p.ocupado && <div className="adventure-world__busy" role="status"><PixelIcon name="dado" size={18} /> Resolvendo sua ação…</div>}
      </div>

      {p.resultado && <p className="adventure-stage__result" role="status">{p.resultado}</p>}
      {p.cena && <div className="adventure-objective">
      ...
      <div className="adventure-console">
        <div className="adventure-console__tabs">
          <button type="button" aria-pressed={aba === 'acoes'} onClick={() => setAba('acoes')}>Comandos</button>
          <button type="button" aria-pressed={aba === 'progressao'} onClick={() => setAba('progressao')}>Jornada 1–{p.progressao?.nivel_maximo ?? 10}</button>
          {foco && <span className="adventure-focus"><PixelIcon name="pocao-azul" size={15} />{foco.nome} {foco.atual}/{foco.maximo}</span>}
        </div>
        {p.erro && <p className="adventure-error" role="alert">{p.erro}</p>}
        {aba === 'acoes' ? <>
          <div className="adventure-console__hint">{p.encerrado ? 'Esta jornada chegou ao fim.' : caido ? 'Você está caído. Avance a rodada para resolver seu teste de morte.' : p.combate ? `Alvo: ${alvo ?? 'nenhum'} · clique em uma criatura para mudar.` : 'Toque nos objetos do cenário para agir ou conte ao Mestre sua ideia.'}</div>
          {p.combate ? <>
            <div className="adventure-actions">
              <button type="button" className="adventure-command adventure-command--primary" disabled={bloqueado || caido || !alvo}
                onClick={() => p.aoAgir({ acao: 'atacar', alvo }, `Atacar ${alvo}`)}><PixelIcon name="espada" size={22} /><span>Atacar<small>Sem custo de foco</small></span></button>
              <button type="button" className="adventure-command" disabled={bloqueado}
                onClick={() => p.aoAgir({ acao: caido ? 'resistir' : 'defender' }, caido ? 'Resistir e avançar a rodada' : 'Defender')}><PixelIcon name="escudo" size={22} /><span>{caido ? 'Resistir' : 'Defender'}<small>{caido ? 'Teste de morte' : 'Proteção e recuperar foco'}</small></span></button>
              <button type="button" className="adventure-command" aria-expanded={taticas} onClick={() => setTaticas(!taticas)}><PixelIcon name="dado" size={22} /><span>Táticas<small>{taticas ? 'Recolher opções' : 'Risco, furtividade, fuga'}</small></span></button>
            </div>
            {taticas && <div className="adventure-actions adventure-actions--tactics">{TATICAS.map(tatica => <button type="button" className="adventure-command" key={tatica.acao} title={tatica.dica} disabled={bloqueado || caido || (tatica.acao === 'investir' && !alvo)}
              onClick={() => p.aoAgir({ acao: tatica.acao, ...(tatica.acao === 'investir' ? { alvo } : {}) }, tatica.nome)}><PixelIcon name={tatica.icone} size={16} /><span>{tatica.nome}<small>{tatica.dica}</small></span></button>)}</div>}
          </> : !p.encerrado && <button type="button" className="adventure-command" disabled={bloqueado || caido}
            onClick={() => p.aoAgir({ acao: 'descansar' }, 'Fazer um descanso curto')}><PixelIcon name="cura" size={20} /><span>Descansar<small>Recupere forças entre encontros</small></span></button>}
          {(p.progressao?.habilidades.length ?? 0) > 0 && <div className="adventure-skills" aria-label={`Habilidades de ${p.classe}`}>
            {p.progressao!.habilidades.map(habilidade => {
              ...
              return <button type="button" className={`adventure-skill ${trancada ? 'adventure-skill--locked' : ''}`} key={habilidade.id}
                ...
                <span className="adventure-skill__heading"><strong>{habilidade.nome}</strong><span>{habilidade.custo} {foco?.nome ?? 'Foco'}</span></span>
                <span className="adventure-skill__description">{habilidade.descricao}</span><small>{motivo}</small>
              </button>;
            })}
          </div>}
        </> : <div className="adventure-progression">
          ...
          <ol className="adventure-progression__levels">{p.progressao?.niveis.map(etapa => <li key={etapa.nivel} className={etapa.nivel <= p.nivel ? 'is-unlocked' : ''} aria-current={etapa.nivel === p.nivel ? 'step' : undefined}>
            <span>{etapa.nivel}</span><div><strong>{etapa.descricao}</strong><small>{etapa.xp} XP{etapa.nivel === p.nivel ? ' · Você está aqui' : etapa.nivel < p.nivel ? ' · Conquistado' : ''}</small></div>
          </li>)}</ol>
          {p.marcos.length > 0 && <div className="adventure-progression__milestones"><h3>As marcas da sua história</h3>{p.marcos.map((marco, indice) => <p key={`${indice}-${marco}`}><PixelIcon name="estrela" size={12} />{marco}</p>)}</div>}
        </div>}
      </div>
    </section>
  );
