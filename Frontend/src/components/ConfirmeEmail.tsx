...
export default function ConfirmeEmail({ email }: { email: string }) {
  ...
  return (
    <div className="h-screen w-screen bg-black flex flex-col items-center justify-center text-center px-4 font-sans">
      <Mail size={48} className="text-rpg-gold mb-4" />
      <h1 className="text-2xl font-rpg text-white mb-2">Confirme seu e-mail</h1>
      ...
      >
        {reenviar.isPending ? <Loader2 size={14} className="animate-spin" /> : null}
        {reenviado ? 'E-mail reenviado' : 'Reenviar e-mail'}
      </button>
      {reenviar.isError && <p className="text-red-500 text-xs mt-2">Não deu para reenviar agora. Tenta de novo em instantes.</p>}
    </div>
  );
}
