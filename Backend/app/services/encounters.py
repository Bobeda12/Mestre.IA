...
CENARIOS: dict[str, dict] = {
    "duelo": {
        "nome": "Entre aço e palavra", "tipo": "duelo",
        "descricao": "Cada abertura conta. A postura do adversário revela a próxima ameaça.",
        "objetivo": "Vencer o confronto ou encontrar uma saída.", "meta": 0,
        "interacoes": [
            {"id": "cobertura", "nome": "Tomar cobertura", "descricao": "+3 de defesa nesta rodada."},
            {"id": "distrair", "nome": "Provocar uma abertura", "descricao": "Teste de Carisma: reduz a precisão inimiga."},
        ],
    },
    "emboscada": {
        "nome": "O caminho estreito", "tipo": "emboscada",
        "descricao": "Destroços cortam a passagem. A poeira pode esconder uma retirada.",
        "objetivo": "Romper a emboscada; o terreno também é uma arma.", "meta": 0,
        "interacoes": [
    ...
    "ritual": {
        "nome": "Os três selos", "tipo": "ritual",
        "descricao": "Três focos alimentam o ritual. Seus guardiões não precisam morrer para que ele termine.",
        "objetivo": "Desfazer 3 selos ou derrotar os guardiões.", "meta": 3,
        "interacoes": [
            {"id": "objetivo", "nome": "Desfazer um selo", "descricao": "Teste de Inteligência ou Sabedoria; 3 sucessos vencem."},
            {"id": "cobertura", "nome": "Usar o altar", "descricao": "+3 de defesa nesta rodada."},
        ],
    },
    "resgate": {
        "nome": "Ninguém fica para trás", "tipo": "resgate",
        "descricao": "Uma pessoa está presa entre os combatentes. Cortar suas amarras abre outra forma de vencer.",
        "objetivo": "Abrir passagem e concluir 3 etapas do resgate, ou vencer o combate.", "meta": 3,
        "interacoes": [
            {"id": "objetivo", "nome": "Avançar o resgate", "descricao": "Teste de Força ou Destreza; 3 sucessos libertam o refém."},
            {"id": "distrair", "nome": "Desviar a atenção", "descricao": "Teste de Carisma: inimigos têm desvantagem nesta rodada."},
        ],
    },
    "cerco": {
        "nome": "A última passagem", "tipo": "cerco",
        "descricao": "A passagem ainda pode ser fechada. Trabalhar no mecanismo deixa você exposto.",
        "objetivo": "Preparar e fechar o portão em 3 etapas, ou derrotar os invasores.", "meta": 3,
        "interacoes": [
            {"id": "objetivo", "nome": "Acionar o mecanismo", "descricao": "Teste de Força ou Inteligência; 3 sucessos encerram o cerco."},
    ...
    "cacada": {
        "nome": "Caçador e presa", "tipo": "cacada",
        "descricao": "Há espaço para atrair o inimigo e usar o chão irregular a seu favor.",
        "objetivo": "Controlar a aproximação e sobreviver ao confronto.", "meta": 0,
        "interacoes": [
            {"id": "armadilha", "nome": "Improvisar uma armadilha", "descricao": "Uma vez: teste de Destreza ou Sabedoria, dano em um alvo."},
...
def painel_cena(c_state: CombatState, w_state: WorldState) -> dict:
    if not c_state.ativo:
        return {
            "tipo": "exploracao", "nome": w_state.local or "Sua jornada",
            "descricao": "Observe os detalhes, converse e tente caminhos próprios. O mundo guarda suas escolhas.",
            "objetivo": "Escolha seu próximo passo.", "progresso": 0, "meta": 0,
            "interacoes": [], "rodada": 0,
        }
    cena = CENARIOS.get(c_state.cenario_id, CENARIOS["duelo"])
    return {
        **cena, "progresso": c_state.progresso_objetivo, "rodada": c_state.rodada,
...
def _teste(executor: "ToolExecutor", atributos: tuple[str, ...], motivo: str) -> bool:
    ...
    dados = DadosRolagem(
        tipo="teste", quem="heroi", d20=resultado.rolagem, bonus=bonus, total=resultado.total,
        cd=cd, sucesso=resultado.sucesso, atributo=atributo, motivo=motivo,
    )
    executor.eventos.append(EventoRolagem(
        f"🎲 {motivo}: {resultado.total} vs CD {cd} — {'sucesso' if resultado.sucesso else 'falha' }.", dados,
    ))
...
def interagir_cenario(executor: "ToolExecutor", interacao: str) -> dict:
    ...
    if interacao == "cobertura":
        ...
        atributos = {
            "ritual": ("inteligencia", "sabedoria"), "resgate": ("forca", "destreza"),
        ...
        if c_state.progresso_objetivo >= cena["meta"]:
            ...
            return {"resultado": "vitoria", "objetivo_concluido": True,
        ...
        if _teste(executor, atributos, "Usar o terreno contra o inimigo"):
            ...
            alvos = [i for i in c_state.inimigos if i.hp > 0]
    ...
    return {"interacao": interacao, "progresso": c_state.progresso_objetivo,
            **executor._resolver_reacao_inimiga()}
